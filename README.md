...
Paths in settings are relative to the clone (or `~/…`). First run writes `platepress/settings.json` locally (gitignored). Workflow `default_comfyUI-API.json` ships in the repo root (UI twin: `default_comfyUI.json`). Models and LoRAs stay wherever Comfy keeps them — set folders in Settings if lists are empty.

Books live as folders under `platepress/books/<id>/` (JSON, walls, stills, plates). That folder is gitignored so live books stay on your machine. The shipped demo is **The Book of Everything** (story, captions, Cast text). No images are bundled. If `books/book_of_everything` is missing, the app copies the demo there. Generate the plates yourself. That book cannot be deleted from the UI.
...
- Cast **Name** is the wall token (`ROOM`, `ANYTHING`, `CHAR1`). Text on: that word becomes the lock. **Image** on: stills are `image1`, `image2`, `image3` in token order. `CHAR1` / `PILOT1` still mean first card, `CHAR2` second. `Anna` after `is` is writer text. Do not write `CHARACTER1` — Krea paints a person.
...
Captions use the same slugs. Check **letter** so Generate paints them on the plate (same size, no extra bar). Untagged lines become a bottom box (`CAP_B`). `CAP_T` is the top box. Tags (`NS`, `NS_6`, …) choose balloon shape and cell. Do not paste ink, closer, or sampler.
...
`platepress/books/`, `platepress/settings.json`, and `JUNK/` are gitignored. Commit the demo under `platepress/demo/book_of_everything/` (no images). Do not commit live books, screenshots, Comfy dumps, checkpoints, or LoRAs. Factory UNET is Krea 2 Turbo. Add your own LoRAs in Settings.
