...
LETTERING_CLOSER = (
    "Bold condensed black comic letters, all caps, no italics, no script in the balloons and caption boxes on the plate. "
...
def render_lettering(beats: list[LetterBeat]) -> str:
    ...
    for b in beats:
        ...
        if b.tag == "NS2" and b.part2:
            ...
            if who:
                clause = (
                    f"Draw {shape}. Bold condensed black comic letters, all caps, no italics, no script, {who} says exactly: \"{q1}\". "
            ...
            clause = f"Draw {shape}. Bold condensed black comic letters, all caps, no italics, no script, exactly: \"{q1}\"."
...
def parse_book(
    ...
    for i, (slug, raw_body) in enumerate(prompt_items, start=1):
        ...
        if use_image and not named:
            pw.append(
                f"{slug}: Image on, no CHAR1/ROOM/token in the wall — no still sent"
            )
